...
import grab.com.thuexetoancau.Utilities.GetAllRegisterCarData;
import grab.com.thuexetoancau.Utilities.SharePreference;
import grab.com.thuexetoancau.Utilities.Utilities;

public class EditDriverActivity extends AppCompatActivity  implements GetAllRegisterCarData.onDataReceived {
    private EditText edtName, edtIdendify,edtCurPass,edtNewPass, edtLicense, edtCarNumber;
    private TextView txtCarMade, txtCarSize, txtCarYear, txtCarType , txtWarn;
    private TextView txtCarModel,edtPhone;
    ...
    private void initComponents() {
        ...
        edtCurPass  = (EditText)    findViewById(R.id.edt_curent_pass);
        ...
        txtCarMade  = (TextView)    findViewById(R.id.edt_car_made);

        txtCarSize  = (TextView)    findViewById(R.id.edt_size);
        txtCarYear  = (TextView)    findViewById(R.id.edt_year);
        txtCarType  = (TextView)    findViewById(R.id.edt_car_type);
    ...
    private void requestCarName() {
        ...
        params.put("keyword", txtCarMade.getText().toString());
    ...
    private boolean checkParams() {
        ...
        if (edtCurPass.getText().toString().equals("")|| edtCurPass.getText().toString() == null){
            txtWarn.setVisibility(View.VISIBLE);
            txtWarn.setText("Bạn chưa nhập mật khẩu hiện tại");
            return true;
        }
        if (edtNewPass.getText().toString().equals("")|| edtNewPass.getText().toString() == null){
            txtWarn.setVisibility(View.VISIBLE);
        ...
        if (!Utilities.md5(edtCurPass.getText().toString()).equals(password)){
            txtWarn.setVisibility(View.VISIBLE);
            txtWarn.setText("Bạn đã nhập sai mật khẩu hiện tại");
            return true;
        }
        return false;
    }
    ...
    private void requestRegisterDriver() {
        ...
        params.put("pass", edtNewPass.getText().toString());
        ...
        dialog.setCancelable(false);
        dialog.show();
        BaseService.getHttpClient().post(Defines.URL_REGISTER_DRIVER, params, new AsyncHttpResponseHandler() {
    ...
    private void getDataFromDriverId() {
        ...
        dialog.setCancelable(false);
        dialog.show();
        BaseService.getHttpClient().post(Defines.URL_GET_DRIVER_BY_ID, params, new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, cz.msebera.android.httpclient.Header[] headers, byte[] responseBody) {
                ...
                try {
                    ...
                    for (int i = 0; i < arrayresult.length(); i++) {
                        JSONObject result = arrayresult.getJSONObject(i);
                        String hoten        = result.getString("name");
                        password            = result.getString("pass");
                        String driverPhone  = result.getString("phone");
                        String tenxe        = result.getString("car_model");
                        String hangxe       = result.getString("car_made");
                        String namxe        = result.getString("car_years");
                        String socho        = result.getString("car_size");
                        String carNumber    = result.getString("car_number");
                        String loaixe       = result.getString("car_type");
                        String cmt          = result.getString("card_identify");
                        String banglai      = result.getString("license");
                        edtName.setText(hoten);
                        edtPhone.setText(driverPhone);
                        txtCarMade.setText(hangxe);
                        txtCarModel.setText(tenxe);
                        txtCarSize.setText(socho);
                        txtCarYear.setText(namxe);
                        txtCarType.setText(loaixe);
                        edtCarNumber.setText(carNumber);
                        edtIdendify.setText(cmt);
                        edtLicense.setText(banglai);
                        requestCarName();
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
